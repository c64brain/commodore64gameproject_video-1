...
@loadBanked
        ...
        lda #%00110101                  ; Switch out BASIC, KERNAL, CHAREN, IO
...
; Copy ROM character set to RAM memory

        loadPointer ZEROPAGE_POINTER_2, CHAR_MEM ; Copy CharPad chset binary file data from address $4800
        jsr CopyChars

        ldx PARAM5                      ; manages which level of data we are pointing to for the map
        lda MAP_ADDRESS,x               ; Get character set from ROM (points to LEVEL_1_MAP = $E000)
        sta ZEROPAGE_POINTER_1
        lda MAP_ADDRESS + 1,x           ; Get character set from ROM (points to LEVEL_1_MAP = $E000)
        sta ZEROPAGE_POINTER_1 + 1

; Copy CharPad binary map data to ZEROPAGE_POINTER_2

        loadPointer ZEROPAGE_POINTER_2, MAP_MEM ; Copy CharPad map binary file data
        jsr CopyChars
...
; CharPad MAP binary file data is stored in ZEROPAGE_POINTER_4
...
DrawMap
        ...
        ; Draw map from left to right top to bottom.

        ldx MAP_Y_POS                   ; Count over X to where first map area is seen
...
@loop
        lda (ZEROPAGE_POINTER_4),y              ; fetch CharPad map data
        ...
        addPointer ZEROPAGE_POINTER_4, 40
        ldy #0
        inx
        cpx #5
        bne @loop
...
; Color memory is stored in ZEROPAGE_POINTER_3
...
; SCREEN1_LINE_OFFSET_TABLE_LO = ZEROPAGE_POINTER_1

        clc                             ; add to Y line address
        adc ZEROPAGE_POINTER_1          ; add data stored at 
        sta ZEROPAGE_POINTER_1
...
@drawloop
        ...
        cpy #12                         ; drawn the 15th character? We're finished
        beq @done
